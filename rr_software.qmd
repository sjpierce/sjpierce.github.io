...
into a powerful, integrated suite of tools. The statistical software 
[R](https://www.r-project.org), paired with 
[RStudio Desktop](https://posit.co/products/open-source/rstudio/) 
[@RStudio-Team-RN8351], is a foundation for a set of tools that can generate
...
While @Mair-RN3387 introduces [R Markdown](https://rmarkdown.rstudio.com/) as a
tool for writing dynamic documents, [Quarto](https://quarto.org/)
[@Allaire-RN8427] is a next-generation tool that replaces R Markdown. Quarto is
a key part of scientific publishing for those who want to do reproducible
research. It can produce documents in many different formats (e.g., HTML, PDF,
Word, PowerPoint, and many more). Combining Quarto with TinyTeX enables
production of very nice PDF files, which are great for distribution and 
archiving. Meanwhile, [Git](https://git-scm.com) [@Torvalds-RN3929] and 
[GitHub](https://github.com/) solve source code version control problems and 
facilitate collaboration on code [@Bryan-RN3900].
...
products that make manuscripts more reproducible. Several research compendiums 
are linked on my [R Packages](packages.qmd) page. 

# Purpose
This document describes the software packages I use and recommend to enable
reproducible research via R packages that serve as research compendiums. It
covers which packages one will need, why they are needed, and how to install
...
want to see how I use these software tools, check out my presentation on
reproducible research [@Pierce-RN8764]. 

# Assumptions
1. You will be using either MS Windows 11 or MacOS. The notes below about MacOS
   are a result of helping a few collaborators who rely on MacOS; they may be 
   less detailed (and less up-to-date) because I don't personally use MacOS.  
...
then *base*, then *Download R-4.6.0 for Windows* or use the 
[direct link to the Windows installer](https://cran.r-project.org/bin/windows/base/R-4.6.0-win.exe). 
...
If you previously were using an older version of R (any version in the 4.5.x
series), you should plan to reinstall all your R packages from scratch under R
4.6.0 or later. The best way to do that is to use a script such as
...
## RTools for Windows, version 4.5.6768 (or later)
Installing [RTools](https://cran.r-project.org/bin/windows/Rtools/) 
[@R-Devel-Core-RN8949] is important because it contains some tools for compiling
software packages and one of the R packages we will use later (the devtools
package) depends on having RTools available. You can download RTools from
...
*RTools45_installer*, or use the 
[direct link to the installer file](https://cran.r-project.org/bin/windows/Rtools/rtools45/files/rtools45-6768-6492.exe). 
Save that installation file to your computer, then run it. 
...
# RStudio Desktop, version 2026.04.0+526 (or later)
[RStudio Desktop](https://posit.co/products/open-source/rstudio/) 
...
## RStudio for Windows
You can use this 
[direct link to the Windows installer](https://download1.rstudio.org/electron/windows/RStudio-2026.04.0-526.exe). 
Save that installation file to your computer’s `Downloads` folder, then run it.

## RStudio for MacOS
You can use this [direct link for the MacOS installer](https://download1.rstudio.org/electron/macos/RStudio-2026.04.0-526.dmg).  

# Quarto, version 1.9.37 (or later) 
...
## Quarto for Windows
You can use this 
[direct link to the Windows installer](https://github.com/quarto-dev/quarto-cli/releases/download/v1.9.37/quarto-1.9.37-win.msi). 
Save that installation file to your computer’s `Downloads` folder, then run it.

## Quarto for MacOS
You can use this 
[direct link to the MacOS installer](https://github.com/quarto-dev/quarto-cli/releases/download/v1.9.37/quarto-1.9.37-macos.pkg). 

# Git, version 2.54.0.1 (or later)
...
[direct link for the Windows installer](https://github.com/git-for-windows/git/releases/download/v2.54.0.windows.1/Git-2.54.0.1-64-bit.exe). 
